 ...
 *
 * Automatically generated by update_isa.py on 2011-10-24.
 *
 */
...
public:
  ...
  enum Opcode {
    ...
    OP_IRDR = 102,      // irdr rd, rs (-> ch)
    OP_MULHWU = 16,     // mulhwu rd, rs, rt (-> ch)
    OP_SYSCALL = 73,    // syscall immed
    OP_IWTR = 64,       // iwtr rs, rt (-> ch)
    OP_LUI = 97,        // lui rd, immed
    OP_WOCHE = 81,      // woche -> ch
    OP_LLI = 78,        // lli rd, immed
    OP_CLZ = 106,       // clz rd, rs (-> ch)
    OP_STW = 80,        // stw rs, rt, immed -> ch
    ...
    OP_SELCH = 105,     // selch rd (-> ch)
    OP_TSTCH = 98,      // tstch rd, rs (-> ch)
    OP_TSTCH_P = 67,    // tstch.p rd, rs (-> ch)
    OP_TSTCHI = 109,    // tstchi rd, immed (-> ch)
    OP_TSTCHI_P = 111,  // tstchi.p rd, immed (-> ch)
    OP_NXIPK = 5        // nxipk
