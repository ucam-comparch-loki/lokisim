...
void Debugger::waitForInput() {
  ...
  while (!sc_core::sc_end_of_simulation_invoked()) {
    ...
    if (input == "") {
      ...
      int parameter = -1;
      if (words.size() > 2)
        parameter = StringManipulation::strToInt(words[2]);
...
void Debugger::setBreakPoint(vector<int>& bps, const ComponentID& memory) {
  ...
  for (uint j=0; j<bps.size(); j++) {
    ...
    if (isBreakpoint(addr)) {
      if (mode == DEBUGGER)
        cout << "Removed breakpoint: " << chip->readWordInternal(memory, addr) << endl;
      removeBreakpoint(addr);
    }
    else {
      if (mode == DEBUGGER)
        cout << "Set breakpoint: " << chip->readWordInternal(memory, addr) << endl;
      addBreakpoint(addr);
    }
  }
...
void Debugger::printStack(const ComponentID& core, const ComponentID& memory) {
  ...
  int stackPointer = chip->readRegisterInternal(core, stackReg);
...
void Debugger::printMemLocations(vector<int>& locs, const ComponentID& memory) {
  ...
  for (uint i=0; i<locs.size(); i++) {
    int val = chip->readWordInternal(memory, locs[i]).toInt();
...
void Debugger::printRegs(vector<int>& regs, const ComponentID& core) {
  ...
  if (regs.size() == 0) {
    ...
    for (uint i=0; i<regs.size(); i++) {
      int regVal = chip->readRegisterInternal(core, regs[i]);
...
void Debugger::executeUntilBreakpoint() {
  if (breakpoints.empty()) {
    ...
    while (!hitBreakpoint && cyclesIdle<maxIdleTime &&
...
void Debugger::finishExecution() {
  ...
  static int cyclesPerStep = 100;

  while (cyclesIdle<maxIdleTime &&
        !Instrumentation::executionFinished() &&
        !sc_core::sc_end_of_simulation_invoked()) {
...
vector<int>& Debugger::parseIntVector(vector<string>& words, bool registers) {
  ...
  for (uint i=0; i<words.size(); i++) {
    ...
    if (location[0] == '+') {
      ...
      for (int j=1; j<num; j++) {
        if (registers)
          locations->push_back(start+j);
        else
          locations->push_back(start + j*BYTES_PER_WORD);
      }

      continue;
    }
    else if (location[0] == 'r')
      location.erase(0,1); // Remove "r" from "rXX"
