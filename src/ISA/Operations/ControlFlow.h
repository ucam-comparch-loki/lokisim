...
// `compute()`. This mix-in must wrap any computation operation.
template<class T>
class InstructionFetch : public HasResult<T> {
public:
  InstructionFetch(Instruction encoded) :
      HasResult<T>(encoded),
      fetchChannel(-1) {
    // Nothing
  }
...
template<class T>
class InBufferJump : public Has1Operand<T> {
public:
