_start:
    ...
    or.eop          r0,  r3,  r0  -> 4      # send cache configuration to core 2
...
tocore1:
    ...
    ori.eop         r0,  r0,  0x20000 -> 3  # Tell Cluster 1 where to store its output
...
# Inner loop
    ...
    ldw         0(r8)         -> 1      # load the pixel (perhaps use ldb?)
    ldw         0x10000(r4)   -> 1      # load the weight for this pixel
...
# Inner loop
    ...
    if!p?mullw  r9,  r9,  r29
    srai        r9,  r9,  15            # the shift is common to both branches
    stw         r9,  0x10000(r8)  -> 1  # store the value
