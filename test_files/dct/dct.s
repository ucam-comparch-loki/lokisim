_start:
    ...
    ifp?ori         r4,  r0,  (0,8,2,3,6)
    ifp?ori         r7,  r0,  (0,8,3,3,6)   # Should derive these from r3/r4 of core 0
    ifp?ori         r9,  r0,  (0,2,2)
    ifp?setchmap    0,   r4                 # instruction memory connection = map 0
    ifp?setchmap    1,   r7                 # data memory connection = map 1
    ori             r0,  r0,  0x01000000 > 0  # tell memory we're about to connect
    ifp?ori         r0,  r0,  (0,1,1) > 0   # connect to instruction channel
    ori             r0,  r0,  0x01000000 > 1  # tell memory we're about to connect
    ...
    ifp?fetch       r0,  core1
    ori             r0,  r0,  0x10000 > 3   # Tell Cluster 1 where its first block is
    ori             r0,  r0,  0x20000 > 3   # Tell Cluster 1 where to store its output
    rmtexecute                    > 2       # start sending instructions to cluster 2
    ifp?ori         r7,  r0,  (0,8,4,3,6)
    ifp?ori         r8,  r0,  (0,8,5,3,6)
    ifp?setchmap    0,   r7                 # instruction memory connection = map 0
    ifp?setchmap    1    r8                 # data memory connection = map 1
    ori             r0,  r0,  0x01000000 > 0  # tell memory we're about to connect
