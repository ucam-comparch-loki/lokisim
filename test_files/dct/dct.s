_start:
    ...
    or              r11, r0,  r0            #
    or              r12, r0,  r0            # TODO:
    or              r13, r0,  r0            # remove when lli has ori semantics
    or              r14, r0,  r0            #
    lli             r11, (0,1,0)
    lli             r12, (0,2,0)
    lli             r13, (0,1,2)
    ...
    or              r0,  r23, r0  -> 3      # send cache configuration to core 1
    or.eop          r0,  r23, r0  -> 4      # send cache configuration to core 2
...
tocore1:
    ...
    lui             r8,  0
    lli             r8,  %lo(core1)
    addu            r0,  r0,  r8      -> 3  # Tell Cluster 1 where its first IPK is
    lui             r8,  %hi(0x10200)
    lli             r8,  %lo(0x10200)
    addu            r0,  r8,  r0      -> 3  # Tell Cluster 1 where its first block is
    lui             r8,  %hi(0x20000)
    lli             r8,  %lo(0x20000)
...
tocore2:
    ...
    ifp?lli         r25, (0,0,1,0,0)
    ifp?addu        r24, ch0, r24           # receive/modify cache configuration...
    ifp?addui       r25, r24, r25           # ...for the channels we will connect to
    ifp?setchmapi   0,   r24                # instruction memory connection = map 0
    ifp?setchmapi   1,   r25                # data memory connection = map 1
    ifp?lui         r9,  0x0100
    ifp?addu        r0,  r9,  r0      -> 0  # tell memory we're about to connect
    ...
    lui             r8,  0
    lli             r8,  %lo(core2)
    addu            r0,  r0,  r8      -> 4  # Tell cluster 2 where its first IPK is
    lui             r8,  %hi(0x30000)
    lli             r8,  %lo(0x30000)
...
# Initialisation: receive parameters
    and         r12, r12, r0            # current row = 0
    and         r13, r13, r0            # current column = 0
    lui         r14, %hi(0x10000)       # index of current weight = 0 (k)
    and         r15, r15, r0            # DCT output = 0 (f_val)
    and         r16, r16, r0            # values added to output so far = 0 (i)

# Inner loop
    slli        r17, r2,  3             # current row * 8 = index of start of row
    ...
    or          r18, ch1, r0            # store whichever value arrives first
    ...
    ifp?or      r19, r15, r0            # if p, want to write sum relatively unchanged
...
# Initialisation: receive parameters
    or          r31, ch0, r0            # wait for location of output block
    or          r30, ch2, r0            # wait for location of input block

    and         r12, r12, r0            # current column = 0
    and         r13, r13, r0            # current row = 0
    lui         r14, %hi(0x10000)       # index of current weight = 0 (k)
    and         r15, r15, r0            # DCT output = 0 (f_val)
    and         r16, r16, r0            # values added to output so far = 0 (i)
...
# Inner loop
    ...
    if!p?ibjmp  -128                    # jump back to start of loop (row = 0)
